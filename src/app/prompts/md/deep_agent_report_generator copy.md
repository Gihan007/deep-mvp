...
- **Industry Ranking Score**  
  The report shall calculate and display a clear, quantifiable ranking (e.g., "2nd of 8 Peers") based on the composite strength of the company's core value metrics.
...
#### Tool 1: `investment_factor_ranking_table_tool`
  ...
  **Output (JSON string)**
  ...
  - Each `set_*` field is a **list** of entries shaped like:
    ...
    - `ranking` is the cached ranking row (e.g. `V_Rating`, `Status`, `Rank`, sub-scores like `V_Quality/V_Value/V_Safety/V_Momentum`).
    - `metrics` is the cached underlying metrics row used to compute the ranking (may be empty depending on cache format).
