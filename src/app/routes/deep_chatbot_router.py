...
class ChatQARequest(BaseModel):
    ...
    base64_files: Optional[List[str]] = []
    base64_audios: Optional[List[str]] = []
    # If true, the Langmanus graph will generate a PDF report (base64) using create_pdf_from_markdown
    report_requred: bool = False
...
class ChatQAResponse(BaseModel):
    ...
    # Some graphs do not generate a PDF report; keep it optional and default to empty.
    report_base64: str = ""
...
@router.post("/api/deep_qa_bot", response_model=ChatQAResponse)
def get_chat_qa(request_data: ChatQARequest, app = Depends(get_app)):
    ...
    try:
        ...
        with get_openai_callback() as cb:
            response = app.deep_chatbot.run({
                "question": request_data.question,
                "session_id": session_id,
                "path_images": path_images,
                "path_files": path_files,
                "path_audios": path_audios,
                "report_requred": request_data.report_requred,
            })
        ...
        # Ensure API contract: always return a string (some graphs don't generate a report)
        if report_base64 is None:
            report_base64 = ""
...
@router.post("/api/deep_qa_bot_stream")
    ...
    try:
        ...
        async def event_gen():
            ...
            try:
                ...
                async for event in app.deep_chatbot.stream({
                    "question": request_data.question,
                    "session_id": session_id,
                    "path_images": path_images,
                    "path_files": path_files,
